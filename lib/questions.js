...
// Basisfragen, die durch eine viel konkretere, dokumentbasierte Frage ersetzt werden
// sollen, sobald die passende Unterlage hochgeladen wurde – damit z. B. nie die generische
// "Wie kommt eine bestimmte Note ... zustande?"-Frage auftaucht, wenn längst ein echtes
// Zeugnis mit echten Noten vorliegt.
const DOC_REDUNDANT_QUESTION_IDS = {
  zeugnis: ["q8"],
};
...
async function generateDocumentQuestions(documentContext, count = MAX_DOCUMENT_QUESTIONS) {
  ...
  const prompt = `Du bereitest ein Bewerbungs-Interviewtraining für einen 14–15-jährigen Lehrstellenbewerber in der Schweiz vor. Du hast Zugriff auf seine tatsächlich hochgeladenen Unterlagen (Volltext unten) und sollst daraus knallharte, konkrete Interviewfragen ableiten – so, wie ein Personalverantwortlicher, der die Unterlagen vorher wirklich gelesen hat und gezielt nachhakt.

VERFÜGBARE KATEGORIE-KEYS: ${catList}

HOCHGELADENE UNTERLAGEN (Volltext):
${documentContext}

REGEL #1 – KONKRET STATT GENERISCH:
Jede Frage MUSS mindestens eine konkrete, wörtlich aus der Unterlage entnommene Tatsache enthalten: einen echten Fachnamen, eine echte Note, ein echtes Semester/Schuljahr, einen echten Firmennamen, ein echtes Zitat aus dem Motivationsschreiben, eine echte Tätigkeit aus dem Schnupperbericht, ein echtes Teilergebnis aus Multicheck/Stellwerktest usw. Wenn du keine konkrete Tatsache aus dem Text nennen kannst, stelle zu diesem Thema KEINE Frage.

Verboten sind Fragen, die auch ohne die Unterlage gestellt werden könnten, z. B. "Wie kommt eine bestimmte Note in einem Fach in deinem Zeugnis zustande?" (generisch, kein konkretes Fach/keine konkrete Note genannt).

BEISPIELE FÜR DIE GEWÜNSCHTE KONKRETHEIT (Vorbild für Stil und Präzision, NICHT wörtlich verwenden):
- Zeugnis, auffällig tiefe Einzelnote: "Du hast in Mathematik nur eine 4, während deine anderen Fächer deutlich besser sind – wie erklärst du dir das?"
- Zeugnis, Notenabfall zwischen Semestern: "Im 1. Semester der 2. Sek warst du in [Fach] noch bei einer [Note], im 2. Semester nur noch bei einer [tiefere Note] – was ist da passiert?"
- Motivationsschreiben, konkrete Aussage: "Du schreibst, dass du dich für [konkrete Aussage/Tätigkeit aus dem Schreiben] interessierst – kannst du das an einem Beispiel zeigen?"
- Firmenbeschrieb/Stellenausschreibung: "[Firmenname] sucht laut Ausschreibung explizit jemanden mit [konkrete Anforderung] – wo bringst du das mit?"
- Schnupperbericht: "Im Schnupperbericht steht, dass du bei [konkrete Tätigkeit/Situation] dabei warst – was hast du dabei genau gemacht und was hat dich überrascht?"
- Multicheck/Stellwerktest: "Im [Teilbereich] hast du laut deinem Testresultat [konkretes Ergebnis/Auffälligkeit] – wie schätzt du das selbst ein?"

REGEL #2 – NUR VORHANDENE UNTERLAGEN:
Stelle NUR Fragen zu Themen, zu denen oben tatsächlich eine Unterlage mit Inhalt vorliegt. Erfinde nichts zu Themen ohne Unterlage (z. B. keine Schnupperlehre-Frage, wenn kein Schnupperbericht vorhanden ist, keine Zeugnisfrage ohne Zeugnis).

Erstelle bis zu ${count} solcher Fragen, verteilt auf passende Kategorien. Kurz, klar, direkt, wie in einem echten Bewerbungsgespräch – aber mit der oben verlangten Konkretheit.
...
export async function buildSessionQuestions(supabase, userId) {
  ...
  // Sobald eine Unterlage hochgeladen wurde, deren Themen die dokumentbasierten Fragen
  // ohnehin viel konkreter abdecken, fliegt die entsprechende generische Basisfrage
  // komplett aus der Auswahl (unabhängig davon, ob die Dokumentfragen-Generierung klappt).
  const presentDocTypes = new Set((docs || []).filter((d) => d.extracted_text).map((d) => d.doc_type));
  const excludedIds = new Set();
  Object.entries(DOC_REDUNDANT_QUESTION_IDS).forEach(([docType, ids]) => {
    if (presentDocTypes.has(docType)) ids.forEach((id) => excludedIds.add(id));
  });
  const questionBank = QUESTION_BANK.filter((q) => !excludedIds.has(q.id));
  ...
  const exhaustedCats = catKeys.filter((cat) => {
    const pool = [...questionBank.filter((q) => q.cat === cat), ...generated.filter((q) => q.cat === cat)];
  ...
  if (exhaustedCats.length) {
    const results = await Promise.all(
      exhaustedCats.map(async (cat) => {
        const existingTexts = [...questionBank.filter((q) => q.cat === cat), ...generated.filter((q) => q.cat === cat)].map(
          (q) => q.text
        );
        try {
  ...
  catKeys.forEach((cat) => {
    ...
    const pool = [...questionBank.filter((q) => q.cat === cat), ...generated.filter((q) => q.cat === cat)].map((q) => ({
      q,
      count: counts[q.id] || 0,
      r: Math.random(),
    }));
